...
def plot_multi_days_scenarios(quantiles: np.array, scenarios: np.array, w_true: np.array, y_true: np.array, dir_path: str, name: str, n_s: int,
                              ylim: float = 1):
    """
    ...
    FONTSIZE = 10
    x_index = [i for i in range(1, 2 * 6 + 1)]
    fig, axs = plt.subplots(2, 1, sharex='col', sharey='row', gridspec_kw={'hspace': 0, 'wspace': 0})
    #axs[0].plot(x_index, scenarios[:2 * 6, :n_s], color='gray', linewidth=1, alpha=0.5)
    axs[0].plot(x_index, quantiles[:2 * 6, 9], color='b', linewidth=2, label='10 %')
    axs[0].plot(x_index, quantiles[:2 * 6, 49], color='k', linewidth=2, label='50 %')
    axs[0].plot(x_index, quantiles[:2 * 6, 89], color='g', linewidth=2, label='90 %')
    axs[0].plot(x_index, y_true[:2 * 6], color='r', linewidth=2, label='wind_obs')
    axs[0].plot(x_index, w_true[:2 * 6], color= 'y', linewidth=2, label= 'NWP')
    axs[0].set_ylim(0, ylim)
    axs[0].tick_params(axis='both', labelsize=FONTSIZE)

    #axs[1].plot(x_index, scenarios[2 * 6:4 * 6, :n_s], color='gray', linewidth=1, alpha=0.5)
    axs[1].plot(x_index, quantiles[2 * 6:4 * 6, 9], color='b', linewidth=2, label='10 %')
    axs[1].plot(x_index, quantiles[2 * 6:4 * 6, 49], color='k', linewidth=2, label='50 %')
    axs[1].plot(x_index, quantiles[2 * 6:4 * 6, 89], color='g', linewidth=2, label='90 %')
    axs[1].plot(x_index, y_true[2 * 6:4 * 6], color='r', linewidth=2, label='wind_obs')
    axs[1].plot(x_index, w_true[2 * 6:4 * 6], color= 'y', linewidth=2, label= 'NWP')
...
def plot_scenarios(quantiles: np.array, scenarios: np.array, y_true: np.array, w_true: np.array, dir_path: str, name: str, n_s: int, n_days:int=1, ylim:list=[0, 1]):
    """
    ...
    """
    FONTSIZE = 15
    x_index = [i for i in range(1, n_days * 6 + 1)]
    plt.figure()
    #plt.plot(x_index, scenarios[:n_days * 6, :n_s], color='gray', linewidth=1, alpha=0.5)
    #plt.plot(x_index, quantiles[:n_days * 6, 9], color='b', linewidth=2, label='10 %')
    plt.plot(x_index, quantiles[:n_days * 6, 49], color='k', linewidth=2, label='50 %')
    #plt.plot(x_index, quantiles[:n_days * 6, 89], color='g', linewidth=2, label='90 %')
    plt.plot(x_index, y_true[:n_days * 6], color='r', linewidth=2, label='obs')
    plt.plot(x_index, w_true[:n_days * 6], color='b', linewidth=2, label='NWF')
...
def crps_per_period(scenarios: np.array, y_true: np.array, max_s: int = 100):
    ...
    crps_averaged = crps_t.reshape(n_d, 6).mean(axis=0)  # averaged CRPS per time period (or by marginal)

    return crps_averaged, crps_t.reshape(n_d, 6)
...
def quantiles_and_evaluation(dir_path: str, s_VS: np.array, s_TEST: np.array, N_q: int, df_y_VS: pd.DataFrame,
                             df_y_TEST: pd.DataFrame, df_w: pd.DataFrame,  name: str, ymax_plf: float, ylim_crps: list, tag: str,
                             nb_zones: int):
    """
    ...
    for z in range(nb_zones):
        plot_multi_days_scenarios(quantiles=q_TEST[6 * nb_days_per_zone * z:6 * nb_days_per_zone * (z + 1), :],
                                  scenarios=s_TEST[6 * nb_days_per_zone * z:6 * nb_days_per_zone * (z + 1), :],
                                  y_true=df_y_TEST.values.reshape(-1)[
                                  6 * nb_days_per_zone * z:6 * nb_days_per_zone * (z + 1)], 
                                  w_true=df_w.values.reshape(-1)[
                                  6 * nb_days_per_zone * z:6 * nb_days_per_zone * (z + 1)],
                                  dir_path=dir_path,
...
def quality_comparison_per_track(dir_path: str, N_q: int, df_y: pd.DataFrame, name: str, ylim_plf: list, ylim_crps: list, tag: str, crps:bool=True):
    ...
    for q, s, model in zip(quantiles_list, scenarios_list, labels):
        plot_multi_days_scenarios(quantiles=q, scenarios=s, y_true=y_true, w_true=w_true,  dir_path=dir_path, name='TEST_zone_1_multi_' + name + '_' + model, n_s=50, ylim=1)
        if tag == 'load':
            ylim = [0.2, 0.9]
        else:
            ylim = [0, 1]
